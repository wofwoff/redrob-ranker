...
| Must-have evidence | ~0.17 | Retrieval / vector-DB / ranking-eval experience. **Primary signal: the description-template tier** (`redrob/description_tiers.py`) — the pool's career descriptions are 44 hand-tiered templates, so a role's description is a near-ground-truth measure of JD-relevance (tier 0 non-technical → tier 5 elite retrieval/ranking-at-scale). Corroborated by `skills` (endorsements × duration) and `skill_assessment_scores`; keyword matching is the fallback for off-vocabulary text. | The JD's absolute needs are exactly the keywords a stuffer lists; the template tier tells "recommendation features lighter than FAANG" (tier 2) apart from "RAG ranking, 50M q/mo, NDCG/MRR/recall@K" (tier 5), which keyword matching cannot. Verified safe: tier ≥3 descriptions occur under real ML titles only. |
